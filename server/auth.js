...
// A weak insecure default is used only in development so the server can start
// without manual setup; production deployments MUST set this variable.
const JWT_SECRET = process.env.JWT_SECRET || (() => {
    if (process.env.NODE_ENV === 'production') {
        throw new Error('JWT_SECRET environment variable is required in production');
    }
    console.warn('[security] JWT_SECRET not set — using insecure dev default. Set JWT_SECRET in .env before deploying.');
    return 'iptv-player-dev-only-secret-DO-NOT-USE-IN-PRODUCTION';
})();
