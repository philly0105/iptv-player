 ...
 */
router.post('/epg/:sourceId/channels',
    validateBody({
 ...
 */
router.get('/stream', proxyLimiter, safeUrl('url'), async (req, res) => {
    const maxRetries = 2;
    let lastError = null;
 ...
 */
router.get('/image', proxyLimiter, safeUrl('url'), async (req, res) => {
    try {
        const { url } = req.query;
 ...
 */
router.get('/vod', proxyLimiter, safeUrl('url'), async (req, res) => {
