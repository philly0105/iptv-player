...
class VideoPlayer {
    ...
    async loadSettingsFromServer() {
        try {
            ...
            try {
                const saved = localStorage.getItem('iptv_player_settings');
                if (saved) {
    ...
    async saveSettings() {
        try {
            ...
            try {
                localStorage.setItem('iptv_player_settings', JSON.stringify(this.settings));
            } catch (localErr) {
    ...
    playHls(url) {
        ...
        if (!isHlsUrl) {
            console.log('[Player] playHls received progressive stream URL. Playing directly on video element:', url);
            this.video.src = url;
            this.video.play().catch(e => {
                if (e.name !== 'AbortError') console.log('[Player] Autoplay prevented:', e);
            });
