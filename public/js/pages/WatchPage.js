...
class WatchPage {
    ...
    init() {
        ...
        document.addEventListener('click', (e) => {
            if (this.qualityMenuOpen &&
                ...
                !this.qualityBadgeEl?.contains(e.target)) {
    ...
    setVolumeFromStorage() {
        const savedVolume = localStorage.getItem('iptv-volume') || '80';
    ...
    setVolume(value) {
        if (this.video) {
            ...
            localStorage.setItem('iptv-volume', value);
            this.updateVolumeUI();
        }
    }
    ...
    populateQualityMenu() {
        ...
        if (levels.length > 1) {
            ...
            // Single-rendition or no-level stream — offer transcode presets instead
    ...
    renderQualityMenu(items) {
        ...
        this.qualityList.querySelectorAll('.captions-option').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                // -1 → HLS auto; numeric strings → level index; 'auto'/'1080p'/etc → transcode preset string
                const value = raw === '-1' ? -1
    ...
    async selectQuality(value) {
        ...
        if (this.qualityMode === 'hls') {
            if (this.hls) {
                this.hls.currentLevel = value; // -1 = adaptive auto
        ...
        if (this.qualitySwitching) return;
        ...
        try {
            ...
            if (value !== 'auto') {
                ...
                delete options.maxResolution; // Remove cap so server uses its saved default
            }
    ...
    renderEpisodes() {
            ...
            return `
                ...
                return `
                                <div class="watch-episode-item ${isActive ? 'active' : ''}" 
                                    ...
                                    <button class="episode-download-btn" data-episode-id="${ep.id}" data-container="${ep.container_extension || 'mp4'}" data-title="${(ep.title || `Episode ${ep.episode_num}`).replace(/"/g, '')}" title="Download" style="margin-left:auto;background:none;border:none;cursor:pointer;color:var(--color-text-muted);padding:4px 8px;flex-shrink:0;" onclick="event.stopPropagation()">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" style="width:16px;height:16px;"><path d="M19 9h-4V3H9v6H5l7 7 7-7zm-8 2V5h2v6h1.17L12 13.17 9.83 11H11zm-6 7h14v2H5v-2z"/></svg>
                                    </button>
                                </div>
                            `;
            }).join('')}
        ...
        this.seasonsContainer.querySelectorAll('.watch-episode-item').forEach(ep => {
            ep.addEventListener('click', (e) => {
                if (e.target.closest('.episode-download-btn')) {
                    const btn = e.target.closest('.episode-download-btn');
                    this.downloadEpisode(btn.dataset.episodeId, btn.dataset.container, btn.dataset.title);
                } else {
                    this.playEpisodeFromList(ep);
                }
            });
        });
    }
    ...
    downloadCurrent() {
        ...
        window.location.href = `/api/proxy/vod?url=${encodeURIComponent(this.streamUrl)}&filename=${encodeURIComponent(safeName)}`;
    }

    async downloadEpisode(episodeId, container, title) {
        try {
            const result = await API.proxy.xtream.getStreamUrl(this.content.sourceId, episodeId, 'series', container || 'mp4');
            if (result?.url) {
                const safeName = (title || 'episode').replace(/[^a-zA-Z0-9_\- ]/g, '_').slice(0, 60);
